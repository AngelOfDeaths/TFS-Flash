...
void ProtocolGame::connectCast(uint32_t playerId, OperatingSystem_t operatingSystem)
{
	...
	sendAddCreature(_player, _player->getPosition(), _player->getTile()->getThingIndex(_player), false);
...
#define dispatchDisconnectClient(err) g_dispatcher.addTask(createTask(std::bind(&ProtocolGame::disconnectClient, this, err)))
	...
	if (sessionKey.empty()) {
		...
		accountName = sessionKey.substr(0, pos);
...
void ProtocolGame::parsePacket(NetworkMessage& msg)
{
	...
	if (!original)
		...
		switch (recvbyte) {
			...
			case 0xF2: praseRuleViolationReport(msg); break;
...
void ProtocolGame::praseRuleViolationReport(NetworkMessage& msg)
{
	if (!original && debug()) {
		return;
	}

	uint8_t reportType = msg.get<uint8_t>();
	uint8_t reason = msg.get<uint8_t>();
	...
	addGameTask(&Game::playerRuleViolationReport, player->getID(), reportType, reason, playerName, comment, translation, statementId);
}
...
void ProtocolGame::sendSkullTime() {
...
void ProtocolGame::parseEquipItem(NetworkMessage& msg) {
	...
	Item* item = player->getItemByClientId(clientId);
	...
	Item* equippedItem = player->getInventoryItem(static_cast<slots_t>(slot));
	if (equippedItem == item) {
		g_game.internalMoveItem(item->getParent(), player, CONST_SLOT_WHEREEVER, item, std::min<uint16_t>(100, item->getItemCount()), nullptr);
	} else {
		g_game.internalMoveItem(item->getParent(), player, slot, item, std::min<uint16_t>(100, item->getItemCount()), nullptr);
	}
}

std::list<Item*> ProtocolGame::getContainerItems(const Container* container) {
	std::list<Item*> itemList;

	for (Item* containerItem : container->getItemList()) {
		itemList.push_back(containerItem);
		if (const Container* cont = containerItem->getContainer()) {
		 	for (Item* contItem : getContainerItems(cont)) {
				itemList.push_back(contItem);
			}
		}
	}
...
void ProtocolGame::sendInventory() {
	std::list<uint16_t> itemIDList;
	for (int index = 1; index <= 10; index++) {
		Item* item = player->getInventoryItem(static_cast<slots_t>(index));
		if (item) {
			itemIDList.push_back(item->getClientID());
			if (const Container* container = item->getContainer()) {
				for (Item* containerItem : getContainerItems(container)) {
					itemIDList.push_back(containerItem->getClientID());
				}
			}
		}
	}
	...
	NetworkMessage msg;
	msg.addByte(0xF5);
	msg.add<uint16_t>(itemIDList.size());

	itemIDList.sort();
	for (uint16_t item : itemIDList) {
		msg.add<uint16_t>(item);
		msg.addByte(0x00);
	 	msg.add<uint16_t>(player->getItemTypeCountByClientId(item));
	}
