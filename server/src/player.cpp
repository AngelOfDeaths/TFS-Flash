...
void Player::setCastPassword(std::string _password) {
	cast.password = _password;
}

void Player::setCastDescription(std::string _description) {
	cast.description = _description;
}
...
uint32_t Player::getItemTypeCountByClientId(uint16_t itemId, int32_t subType /*= -1*/) const
{
	uint32_t count = 0;
	for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; i++) {
		Item* item = inventory[i];
...
Item* Player::getItemByClientId(uint16_t itemId, int32_t subType /*= -1*/) const
{
	Item* returnItem;
	for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; i++) {
		Item* item = inventory[i];
		...
		if (Container* container = item->getContainer()) {
			for(Item* contItem : client->getContainerItems(container)) {
				if(contItem->getClientID() == itemId) {
					return contItem;
				}
			}
		}
