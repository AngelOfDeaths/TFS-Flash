...
void LuaScriptInterface::registerFunctions()
{
	...
	// Cast system
	registerMethod("Player", "getCast", LuaScriptInterface::luaPlayerGetCast);
	...
	registerMethod("Player", "setCastState", LuaScriptInterface::luaPlayerSetCastState);
	registerMethod("Player", "setCastDescription", LuaScriptInterface::luaPlayerSetCastDescription);
...
// Cast system
int32_t LuaScriptInterface::luaPlayerGetCast(lua_State* L)
{
	// player:getCast()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_createtable(L, 2, 0);
		setField(L, "status", player->getCastingState() ? 1 : 0);
...
int32_t LuaScriptInterface::luaPlayerGetCastViewers(lua_State* L)
{
...
int32_t LuaScriptInterface::luaPlayerSetCastPassword(lua_State* L)
{
...
int32_t LuaScriptInterface::luaPlayerSetCastState(lua_State* L)
{
	// player:setCastState(boolean)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		bool state = getBoolean(L, 2);
		if (state) {
			if (player->cast.mCastChannelId == -1) {
				ChatChannel* tmpchat = (g_chat->createChannel(*player, CHANNEL_CAST));
				if (!tmpchat) {
					return 0;
				}

				tmpchat->addUser(*player);
				player->cast.mCastChannelId = tmpchat->getId();

			}
			player->sendCreatePrivateChannel(player->cast.mCastChannelId, "Spectator Chat");
		} else {
			int id = player->cast.mCastChannelId;
			player->kickCastViewers();
			g_game.playerCloseChannel(player->id, id);
		}
...
int32_t LuaScriptInterface::luaPlayerSetCastDescription(lua_State* L)
{
	// player:setCastDescription()
	Player* player = getUserdata<Player>(L, 1);
	std::string _description = popString(L);
	if (player) {
		player->setCastDescription(_description);
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int32_t LuaScriptInterface::luaPlayerIsCasting(lua_State* L)
{
	// player:isCasting()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		pushBoolean(L, player->getCastingState());
	} else {
		lua_pushnil(L);
	}
	return 1;
