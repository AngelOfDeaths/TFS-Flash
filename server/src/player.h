...
class Player final : public Creature, public Cylinder
{
	public:
		...
		bool hasCastPassword() {
			return cast.password != "";
		}

		PlayerCast getCast();
		void setCastPassword(std::string password_);
		void setCastDescription(std::string _description);
		std::string getCastDescription() {
		...
		void sendCreatureInventory(){
			for (auto it : cSpectators) {
				it.second->sendInventory();
			}
		...
		void onAttackedCreatureDisappear(bool isLogout, const Creature* creature) final;
		void onFollowCreatureDisappear(bool isLogout, const Creature* creature) final;
		...
		void sendCancelMessage(const std::string& msg) const {
			for (auto it : cSpectators) {
				it.second->sendTextMessage(TextMessage(MESSAGE_STATUS_SMALL, msg));
			}
		...
		void sendCancelTarget(uint32_t creatureId) const {
			for (auto it : cSpectators) {
				it.second->sendCancelTarget(creatureId, getOperatingSystem());
			}
		...
		void sendTextMessage(MessageClasses mclass, const std::string& message) const {
			for (auto it : cSpectators) {
				it.second->sendTextMessage(TextMessage(mclass, message));
			}
		...
		void sendTradeItemRequest(const std::string& traderName, const Item* item, bool ack) const {
			for (auto it : cSpectators) {
				it.second->sendTradeItemRequest(traderName, item, ack);
			}
	...
	protected:
		...
		uint32_t getItemTypeCountByClientId(uint16_t itemId, int32_t subType = -1) const;
		Item* getItemByClientId(uint16_t itemId, int32_t subType = -1) const;
