...
void Game::playerCloseChannel(uint32_t playerId, uint16_t channelId)
{
	...
	if (player->cast.isCasting && channelId == player->cast.mCastChannelId) {
...
void Game::playerUpdateHouseWindow(uint32_t playerId, uint8_t listId, uint32_t windowTextId, const std::string& text)
{
	...
	if ((house && internalWindowTextId == windowTextId && listId == 0) || (player->getOperatingSystem() == CLIENTOS_FLASH && listId == 255)) {
...
void Game::playerRuleViolationReport(uint32_t playerId, uint8_t reportType, uint8_t reason, const std::string& playerName, const std::string& comment, const std::string& translation, uint32_t statementId)
{
	Player* player = getPlayerByID(playerId);
	if (!player) {
		return;
	...
	Database* db = Database::getInstance();
	std::ostringstream query;
	query << "INSERT INTO `rule_violations` (`reportType`, `reason`, `reportedBy`, `playerName`, `comment`, `translation`, `statementId`) VALUES ('"
	<< unsigned(reportType) << "','" << unsigned(reason) << "','" << player->getGUID() << "','" << playerName << "'," << db->escapeString(comment) << ",'" << translation << "','" << statementId << "')";
	Database::getInstance()->executeQuery(query.str());

	std::ostringstream str;
	str << "Thank you for reporting " << playerName << ". Your report will be processed by customer support as soon as possible. You can find the result of your report at your account page.";
	player->sendTextMessage(MESSAGE_REPORT, str.str());
}
