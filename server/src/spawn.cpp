...
void Spawn::sendSpawnEffect(const Position& pos){
	SpectatorVec list;
	g_game.map.getSpectators(list, pos, false, true);
	for(Creature* spectator : list){
		spectator->getPlayer()->sendMagicEffect(pos, CONST_ME_TELEPORT);
	}
}

void Spawn::sendSpawnRest(uint32_t spawnId, spawnBlock_t& sb){
...
void Spawn::checkSpawn()
{
	...
	for (auto& it : spawnMap) {
		...
		if (OTSYS_TIME() >= (sb.lastSpawn + sb.interval) - 6000) {
			if (findPlayer(sb.pos)) {
				sb.lastSpawn = OTSYS_TIME();
				sendSpawnEffect(sb.pos);
				g_scheduler.addEvent(createSchedulerTask(2000, std::bind(&Spawn::sendSpawnEffect, this, sb.pos)));
				g_scheduler.addEvent(createSchedulerTask(4000, std::bind(&Spawn::sendSpawnEffect, this, sb.pos)));
			}

			g_scheduler.addEvent(createSchedulerTask(6000, std::bind(&Spawn::sendSpawnRest, this, spawnId, sb)));
